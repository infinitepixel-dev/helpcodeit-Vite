...
function BlogsPage() {
    ...
    if (!posts || posts.length === 0) {
      ...
      return (
        <div>
          <h1 className='my-8 text-center text-7xl satisfyFont'>Code Chronicles</h1>
          <p className='text-3xl text-center'>Welcome to the Help Code It blog </p>
          <div className="container">
            <h2 className="mb-5 text-3xl font-bold text-balance mt-14">Recent Posts</h2>
            <ul>
              {posts.map((post) => {
            ...
            let publishedDate = format(new Date(post.sys.updatedAt), 'MMMM dd, yyyy');
                return (
                <li key={post.sys.id}>
                  <div className="flex flex-col justify-between p-6 mb-8 bg-orange-300 rounded shadow-md dark:shadow-none dark:bg-slate-500">
                                      <div>
                                      ...
                                      <Link
                                          ...
                                          className="self-start mt-2 font-bold text-blue-600 dark:text-blue-400 hover:underline"
                                      >
                                          Read This Post
                                      </Link>
                                  </div>
